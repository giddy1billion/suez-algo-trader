...
def cmd_status(broker: AlpacaBroker):
    ...
    print(f"  PDT Flag:      {'Yes ⚠️' if account['pattern_day_trader'] else 'No ✅'}")
    print(f"{'─'*50}")
    ...
    if positions:
        ...
        for p in positions:
            ...
            print(f"  {pnl_emoji} {p['symbol']:8s} | {p['qty']:>8.4f} @ ${p['avg_entry_price']:.2f} "
...
def cmd_backtest(broker: AlpacaBroker, strategy_name: str, symbols: list[str], timeframe: str, lookback: int):
    ...
    for symbol in symbols:
        ...
        if df is None or len(df) < 100:
            print(f"  ⚠️ Insufficient data for {symbol} (got {len(df) if df is not None else 0} bars)")
            continue
...
def cmd_train(broker: AlpacaBroker, symbols: list[str], timeframe: str):
    ...
    if training_data:
        ...
        print(f"\n✅ Model trained and saved to {settings.ml_model_path}")
    else:
...
def cmd_run(
    ...
    print(f"\n✅ Bot stopped after {cycle_count} cycles. All state saved.")
